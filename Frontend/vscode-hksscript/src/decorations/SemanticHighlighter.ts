...
const COLORS = {
    keyword:   { color: '#b784e0', fontWeight: 'bold' as const },
    type:      { color: '#7ecf7e' },
    function:  { color: '#e8c86a' },
    variable:  { color: '#7ab8e0' },
    comment:   { color: '#4a8a4a', fontStyle: 'italic' as const },
    string:    { color: '#6aa8e0' },
    number:    { color: '#e09860' },
};

export class SemanticHighlighter {
    private decKeyword: vscode.TextEditorDecorationType;
    private decType: vscode.TextEditorDecorationType;
    private decFunc: vscode.TextEditorDecorationType;
    private decVar: vscode.TextEditorDecorationType;
    private decComment: vscode.TextEditorDecorationType;
    private decString: vscode.TextEditorDecorationType;
    private decNumber: vscode.TextEditorDecorationType;
    ...
    constructor(context: vscode.ExtensionContext) {
        ...
        context.subscriptions.push(this.decKeyword, this.decType, this.decFunc, this.decVar,
            this.decComment, this.decString, this.decNumber);
    }

    update(editor: vscode.TextEditor) {
        const text = editor.document.getText();
        const posAt = (offset: number) => editor.document.positionAt(offset);
        const kwR: vscode.Range[] = []; const tpR: vscode.Range[] = [];
        const fnR: vscode.Range[] = []; const vaR: vscode.Range[] = [];
        const coR: vscode.Range[] = []; const stR: vscode.Range[] = [];
        ...
        while ((m = callRe.exec(text)) !== null) { if (!skip.has(m[1])) fnR.push(new vscode.Range(posAt(m.index), posAt(m.index + m[1].length))); }
        // variables
